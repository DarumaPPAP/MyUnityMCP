# UnityGraphicsMCP Phase 4B Dirty Dependency Bake
...
Phase 3 MutationとPhase 4A Saveの結果から、再生成が必要なGraphics依存をEditor Session内で追跡し、明示承認されたDependencyだけをBakeする。
...
APVはDirty Dependencyとして検出できるが、Phase 4Bでは実行しない。
