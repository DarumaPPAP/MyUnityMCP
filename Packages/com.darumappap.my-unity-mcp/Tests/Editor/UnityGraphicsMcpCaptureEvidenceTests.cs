...
namespace UnityGraphicsMcp
{
	public sealed class UnityGraphicsMcpCaptureEvidenceTests
	{
		...
		public void Bridge_DiscoversCaptureEvidenceCaptureTools_AndKeepsThemDisabled()
		{
			...
			Assert.That(GetToolAttribute(typeof(GraphicsCaptureEvidenceTool)).AutoRegister, Is.False);
			Assert.That(GetToolAttribute(typeof(GraphicsSubmitVisualReviewTool)).AutoRegister, Is.False);
			Assert.That(GetToolAttribute(typeof(GraphicsRefineFromVisualReviewTool)).AutoRegister, Is.False);
		}

		[Test]
		public void ObjectIdEncoding_IsDeterministic24Bit()
		{
			Color32 first = UnityGraphicsMcpInspection.EncodeCaptureEvidenceObjectIdForTests(1);
			Color32 second = UnityGraphicsMcpInspection.EncodeCaptureEvidenceObjectIdForTests(256);
			Color32 third = UnityGraphicsMcpInspection.EncodeCaptureEvidenceObjectIdForTests(65536);
		...
		{
			UnityGraphicsMcpCaptureEvidenceRecord first = CreateDigestRecord();
			UnityGraphicsMcpCaptureEvidenceRecord second = CreateDigestRecord();
			second.Artifacts.Reverse();

			string firstDigest = UnityGraphicsMcpInspection.BuildCaptureEvidenceEvidenceDigestForTests(first);
			string secondDigest = UnityGraphicsMcpInspection.BuildCaptureEvidenceEvidenceDigestForTests(second);
			...
			Assert.That(
				UnityGraphicsMcpInspection.BuildCaptureEvidenceEvidenceDigestForTests(second),
		...
		public void CaptureEvidence_RejectsUnsupportedChannel()
		{
			...
			UnityGraphicsMcpToolResult result = UnityGraphicsMcpInspection.CaptureEvidence(
		...
		public void SubmitVisualReview_RequiresExactEvidenceDigest()
		{
			...
			UnityGraphicsMcpToolResult result = UnityGraphicsMcpInspection.SubmitVisualReview(
		...
		{
			string captureId = StoreFakeCapture("digest");
			UnityGraphicsMcpToolResult accepted = UnityGraphicsMcpInspection.SubmitVisualReview(
			...
			UnityGraphicsMcpToolResult second = UnityGraphicsMcpInspection.SubmitVisualReview(
		...
		{
			string captureId = StoreFakeCapture("digest");
			UnityGraphicsMcpToolResult result = SubmitNeedsAdjustment(captureId, "digest");
			Assert.That(result.IsSuccessful, Is.True, result.summary);
			Dictionary<string, object> data = ResultData(result);
		...
		public void RefineFromVisualReview_CreatesPlanForNeedsAdjustment()
		{
			...
			UnityGraphicsMcpToolResult result = UnityGraphicsMcpInspection.RefineFromVisualReview(
		...
		private static UnityGraphicsMcpCaptureEvidenceRecord CreateDigestRecord()
		{
			return new UnityGraphicsMcpCaptureEvidenceRecord
			{
				...
				Artifacts = new List<UnityGraphicsMcpCaptureArtifactRecord>
				{
		...
		{
			UnityGraphicsMcpCaptureEvidenceRecord capture = new UnityGraphicsMcpCaptureEvidenceRecord
			{
			...
			return UnityGraphicsMcpCaptureEvidenceSession.StoreCaptureForTests(capture);
		}

		private static UnityGraphicsMcpToolResult SubmitNeedsAdjustment(string captureId, string digest)
		{
