...
{
	public sealed class EnvironmentMutationTests
	{
		...
		public void SetUp()
		{
			...
			Session.ClearPlans();
			EnvironmentMutationSession.ClearForTests();
			Undo.ClearAll();
		}
		...
		public void Bridge_DiscoversEnvironmentTools_AndKeepsThemDisabledByDefault()
		{
			...
			Assert.That(GetToolAttribute(typeof(PrepareEnvironmentPlanTool)).AutoRegister, Is.False);
			Assert.That(GetToolAttribute(typeof(ApplyEnvironmentPlanTool)).AutoRegister, Is.False);
			Assert.That(GetToolAttribute(typeof(UndoLastEnvironmentTransactionTool)).AutoRegister, Is.False);
		}
		...
		public void ApplyEnvironmentPlan_RejectsMissingApproval()
		{
			...
			ToolResult result = Inspection.ApplyEnvironmentPlan(
		...
		public void ApplyAndUndo_CreatesCamera()
		{
			...
			ToolResult applyResult = Apply("environment-camera-apply", executable);
			...
			ToolResult undoResult = UndoEnvironment("environment-camera-undo", applyResult);
		...
		public void ApplyAndUndo_UpdatesCamera()
		{
			...
			ToolResult applyResult = Apply("environment-camera-update-apply", executable);
			Assert.That(applyResult.IsSuccessful, Is.True, applyResult.summary);
			...
			ToolResult undoResult = UndoEnvironment("environment-camera-update-undo", applyResult);
			Assert.That(undoResult.IsSuccessful, Is.True, undoResult.summary);
		...
		public void ApplyAndUndo_CreatesReflectionProbe()
		{
			...
			ToolResult applyResult = Apply("environment-probe-apply", executable);
			Assert.That(applyResult.IsSuccessful, Is.True, applyResult.summary);
			...
			ToolResult undoResult = UndoEnvironment("environment-probe-undo", applyResult);
			Assert.That(undoResult.IsSuccessful, Is.True, undoResult.summary);
		...
		public void ApplyAndUndo_UpdatesReflectionProbe()
		{
			...
			ToolResult applyResult = Apply("environment-probe-update-apply", executable);
			Assert.That(applyResult.IsSuccessful, Is.True, applyResult.summary);
			...
			ToolResult undoResult = UndoEnvironment("environment-probe-update-undo", applyResult);
			Assert.That(undoResult.IsSuccessful, Is.True, undoResult.summary);
		...
		public void ApplyAndUndo_CreatesVolume_WhenCorePackageIsAvailable()
		{
			...
			ToolResult applyResult = Apply("environment-volume-apply", executable);
			Assert.That(applyResult.IsSuccessful, Is.True, applyResult.summary);
			...
			ToolResult undoResult = UndoEnvironment("environment-volume-undo", applyResult);
			Assert.That(undoResult.IsSuccessful, Is.True, undoResult.summary);
		...
		public void ApplyAndUndo_UpdatesVolumeAndSharedProfile()
		{
			...
			ToolResult applyResult = Apply("environment-volume-update-apply", executable);
			Assert.That(applyResult.IsSuccessful, Is.True, applyResult.summary);
			Assert.That((bool)GetMemberValue(volume, "isGlobal"), Is.True);
			...
			ToolResult undoResult = UndoEnvironment("environment-volume-update-undo", applyResult);
			Assert.That(undoResult.IsSuccessful, Is.True, undoResult.summary);
		...
		public void ApplyEnvironmentPlan_RejectsChangedBaseline()
		{
			...
			ToolResult result = Apply("environment-baseline-apply", executable);
		...
		public void ApplyEnvironmentPlan_IsAtomicAcrossCameraProbeAndVolume()
		{
			...
			ToolResult applyResult = Apply("environment-atomic-apply", executable);
			Assert.That(applyResult.IsSuccessful, Is.True, applyResult.summary);
			...
			ToolResult undoResult = UndoEnvironment("environment-atomic-undo", applyResult);
			Assert.That(undoResult.IsSuccessful, Is.True, undoResult.summary);
			Assert.That(FindAll<Camera>().Length, Is.EqualTo(0));
			Assert.That(FindAll<ReflectionProbe>().Length, Is.EqualTo(0));
			Assert.That(FindComponent(volumeType), Is.Null);
		...
		public void UndoLastEnvironmentTransaction_RejectsExternalTargetChange()
		{
			...
			ToolResult applyResult = Apply("environment-external-change-apply", executable);
			Assert.That(applyResult.IsSuccessful, Is.True, applyResult.summary);
			Camera camera = FindAll<Camera>()[0];
			camera.fieldOfView = 23.0f;

			ToolResult undoResult = UndoEnvironment("environment-external-change-undo", applyResult);
		...
		public void UndoLastEnvironmentTransaction_RejectsNewerUndoGroup()
		{
			...
			ToolResult applyResult = Apply("environment-newer-undo-apply", executable);
			Assert.That(applyResult.IsSuccessful, Is.True, applyResult.summary);

			TestAsset marker = ScriptableObject.CreateInstance<TestAsset>();
			Undo.IncrementCurrentGroup();
			Undo.RecordObject(marker, "Unrelated Newer Undo Group");
			marker.value = 10;

			ToolResult undoResult = UndoEnvironment("environment-newer-undo-result", applyResult);
			Assert.That(undoResult.status, Is.EqualTo(E_MCP_TOOL_STATUS.INVALID_REQUEST.ToString()));
			Assert.That(FindAll<Camera>().Length, Is.EqualTo(1));
			UnityEngine.Object.DestroyImmediate(marker);
		}

		private static Dictionary<string, object> PrepareSingle(string requestId, EnvironmentOperationInput operation)
		{
			Dictionary<string, object> direction = CompileDirection(requestId + "-direction");
			ToolResult result = Prepare(requestId + "-prepare", direction, new[] { operation });
			Assert.That(result.IsSuccessful, Is.True, result.summary);
		...
		{
			return Inspection.PrepareEnvironmentPlan(
				requestId,
		...
		private static ToolResult Apply(string requestId, Dictionary<string, object> executable)
		{
			return Inspection.ApplyEnvironmentPlan(
				requestId,
				executable["planId"] as string,
				Convert.ToInt64(executable["expectedRevision"]),
				executable["approvalToken"] as string,
		...
		private static ToolResult UndoEnvironment(string requestId, ToolResult applyResult)
		{
			Dictionary<string, object> applyData = ResultData(applyResult);
			return Inspection.UndoLastEnvironmentTransaction(
				requestId,
		...
		private static EnvironmentOperationInput CreateCameraOperation(string operationId)
		{
			return new EnvironmentOperationInput
			{
				...
				backgroundColor = new ColorInput { r = 0.05f, g = 0.06f, b = 0.08f, a = 1.0f },
