...
namespace UnityGraphicsMcp
{
	...
	public static partial class Session
	{
		...
		private static readonly Dictionary<string, SceneSnapshot> _snapshots =
			new Dictionary<string, SceneSnapshot>();
		private static readonly Dictionary<string, DirectionPlan> _plans =
			new Dictionary<string, DirectionPlan>();
		...
		public static ReadOnlyGuard BeginReadOnlyGuard()
		{
		...
		public static string StoreSnapshot(SceneSnapshot snapshot)
		{
			if (snapshot == null)
			{
		...
		public static string StorePlan(DirectionPlan plan)
		{
			if (plan == null)
			{
		...
		private static Dictionary<ulong, bool> CaptureSceneDirtyState()
		{
			...
			for (int index = 0; index < SceneManager.sceneCount; index++)
			{
				...
				if (scene.IsValid())
				{
					states[IdentityCompatibility.GetSceneHandle(scene)] =
						scene.isDirty;
				}
			}
		...
		private static void RemoveExpiredSnapshots()
		{
			...
			foreach (KeyValuePair<string, SceneSnapshot> pair in _snapshots)
			{
		...
		private static void RemoveExpiredPlans()
		{
			...
			foreach (KeyValuePair<string, DirectionPlan> pair in _plans)
			{
				if (pair.Value.CreatedUtc >= threshold)
				{
					continue;
	...
	public sealed class ReadOnlyGuard
	{
		...
		public bool HasViolation(out Dictionary<string, object> evidence)
		{
			...
			for (int index = 0; index < SceneManager.sceneCount; index++)
			{
				...
				bool beforeDirty;
				ulong sceneHandle =
					IdentityCompatibility.GetSceneHandle(scene);
