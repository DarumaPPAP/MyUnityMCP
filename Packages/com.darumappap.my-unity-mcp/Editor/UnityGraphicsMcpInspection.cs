...
namespace UnityGraphicsMcp
{
	...
	public sealed class UnityGraphicsMcpIssue
	{
	...
	public sealed class UnityGraphicsMcpFinding
	{
	...
	public sealed class UnityGraphicsMcpSceneItem
	{
	...
	public sealed class UnityGraphicsMcpSceneSnapshot
	{
		public string SnapshotId { get; set; }
		public long Revision { get; set; }
		public DateTime CreatedUtc { get; set; }
		public Dictionary<string, object> Summary { get; set; } = new Dictionary<string, object>();
		public List<UnityGraphicsMcpSceneItem> Items { get; set; } = new List<UnityGraphicsMcpSceneItem>();
	}
	...
	public static partial class UnityGraphicsMcpInspection
	{
		...
		public static UnityGraphicsMcpToolResult InspectProject(string requestId)
		{
		...
		public static UnityGraphicsMcpToolResult InspectScene(
			string requestId,
			...
			string cursor)
		{
			return ExecuteReadOnly(
				...
				delegate
				{
					...
					UnityGraphicsMcpSceneSnapshot snapshot;
					if (!string.IsNullOrWhiteSpace(snapshotId))
					{
						E_MCP_TOOL_STATUS failureStatus;
						if (!UnityGraphicsMcpSession.TryGetSnapshot(snapshotId, out snapshot, out failureStatus))
						{
							return CreateResult(
								"graphics.inspect_scene",
								requestId,
					...
					else
					{
						...
						UnityGraphicsMcpSession.StoreSnapshot(snapshot);
					}
					...
					List<UnityGraphicsMcpSceneItem> items = snapshot.Items
						.Skip(cursorIndex)
						.Take(pageSize)
						.ToList();
		...
		public static UnityGraphicsMcpToolResult ValidateScene(
			string requestId,
			bool includeInactive)
		{
			return ExecuteReadOnly(
				...
				delegate
				{
					List<UnityGraphicsMcpFinding> findings = BuildValidationFindings(includeInactive);
		...
		private static UnityGraphicsMcpToolResult ExecuteReadOnly(
			string toolName,
			string requestId,
			Func<UnityGraphicsMcpToolResult> operation)
		{
			...
			UnityGraphicsMcpReadOnlyGuard guard = UnityGraphicsMcpSession.BeginReadOnlyGuard();
		...
		private static UnityGraphicsMcpToolResult CreateResult(
			string toolName,
			string requestId,
			...
			object data)
		{
			return new UnityGraphicsMcpToolResult
			{
				tool = toolName,
