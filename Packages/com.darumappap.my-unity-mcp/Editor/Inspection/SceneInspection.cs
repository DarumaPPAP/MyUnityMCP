...
namespace UnityGraphicsMcp
{
	...
	{
		private static SceneSnapshot BuildSceneSnapshot(
			...
			HashSet<int> shaderIds)
		{
			...
			for (int index = 0; index < materials.Length; index++)
			{
				...
				materialIds.Add(IdentityCompatibility.GetObjectToken(material));
				Shader shader = material.shader;
				if (shader != null)
				{
					shaderIds.Add(IdentityCompatibility.GetObjectToken(shader));
				}
