...
namespace UnityGraphicsMcp
{
	...
	public sealed class BakeTargetInput
	{
	...
	internal sealed class DirtyDependencyRecord
	{
		public string ScenePath { get; set; }
	...
	internal sealed class BakeSceneBaseline
	{
		public int SceneHandle { get; set; }
		public string ScenePath { get; set; }
	...
	internal sealed class PreparedBakeDependency
	{
	...
	internal sealed class ExecutableBakePlan
	{
		...
		public List<BakeSceneBaseline> ContributingScenes { get; set; } =
			new List<BakeSceneBaseline>();
		public List<PreparedBakeDependency> Dependencies { get; set; } =
			new List<PreparedBakeDependency>();
	}

	[InitializeOnLoad]
	internal static class DependencyBakeSession
	{
		private const int MAX_BAKE_PLAN_COUNT = 8;
		private static readonly TimeSpan BAKE_PLAN_LIFETIME = TimeSpan.FromMinutes(10.0);
		private static readonly Dictionary<string, ExecutableBakePlan> _plans =
			new Dictionary<string, ExecutableBakePlan>();
		private static readonly Dictionary<string, DirtyDependencyRecord>
			_dirtyDependencies =
				new Dictionary<string, DirtyDependencyRecord>(
		...
		public static string StorePlan(ExecutableBakePlan plan)
		{
			RemoveExpiredPlans();
			...
			out string failureMessage)
		{
			...
			if (string.IsNullOrWhiteSpace(approvalToken) ||
				!string.Equals(
					...
					SaveEvaluationSession.HashText(approvalToken),
					StringComparison.Ordinal))
			{
				failureStatus = E_MCP_TOOL_STATUS.INVALID_REQUEST;
		...
		public static void ConsumePlan(ExecutableBakePlan plan)
		{
			if (plan != null)
			{
				plan.Consumed = true;
		...
		private static void TrackDirtyScene(Scene scene)
		{
			...
			{
				record = new DirtyDependencyRecord
				{
			...
			if (record.SceneHandle != IdentityCompatibility.GetSceneToken(scene))
			{
				record.SceneHandle = IdentityCompatibility.GetSceneToken(scene);
				dependencySetChanged = true;
			}
	...
	public static partial class Inspection
	{
		...
		public static ToolResult PrepareBakePlan(
			string requestId,
			long? expectedRevision,
			BakeTargetInput[] targets)
		{
			return ExecuteReadOnly(
				...
				delegate
				{
					...
					DependencyBakeSession.EnsureCurrentlyDirtyScenesTracked();

					List<BakeSceneBaseline> contributingScenes;
					ToolResult sceneSetFailure;
					...
					List<PreparedBakeDependency> dependencies =
						new List<PreparedBakeDependency>();
					HashSet<string> dependencyKeys =
						new HashSet<string>(StringComparer.Ordinal);

					foreach (BakeTargetInput target in targets)
					{
						ToolResult targetFailure;
					...
					ExecutableBakePlan plan =
						new ExecutableBakePlan
						{
					...
					DependencyBakeSession.StorePlan(plan);
		...
		public static ToolResult BakeDependencies(
			string requestId,
			string planId,
			long? expectedRevision,
			string approvalToken,
			string bakeMode)
		{
			return ExecuteSaveEvaluationPersistentOperation(
				...
				delegate
				{
					...
					if (!DependencyBakeSession.TryGetPlan(
						planId,
					...
					if (!PreflightDependencyBakeDependencies(
						...
						out preflightIssues))
					{
						ToolResult unsupportedResult = CreateResult(
							"graphics.bake_dependencies",
							requestId,
		...
		private static ToolResult ExecuteDependencyBakePlan(
			string requestId,
			ExecutableBakePlan plan)
		{
			...
			DependencyBakeSession.ConsumePlan(plan);
			DependencyBakeSession.BeginOwnedBake();

			try
			{
				foreach (PreparedBakeDependency dependency
					in plan.Dependencies)
				{
					...
					DependencyBakeSession.ClearCompletedDependency(
						dependency);
				}
			}
			finally
			{
				DependencyBakeSession.EndOwnedBake();
			}
		...
		private static bool TryCaptureDependencyBakeContributingScenes(
			string requestId,
			out List<BakeSceneBaseline> scenes,
			out ToolResult failure)
		{
			scenes = new List<BakeSceneBaseline>();
			failure = null;
			...
			for (int index = 0; index < SceneManager.sceneCount; index++)
			{
				...
				if (!IsSupportedSaveEvaluationSceneAssetPath(path))
				{
					failure = CreateResult(
						...
						new Dictionary<string, object>
						{
							{ "sceneHandle", IdentityCompatibility.GetSceneToken(scene) },
							{ "scenePath", path }
						});
					return false;
				}
			...
			out ToolResult failure)
		{
			...
			DirtyDependencyRecord dirtyRecord;
			if (!DependencyBakeSession.TryGetDirtyRecord(
				scenePath,
				out dirtyRecord))
			{
				failure = CreateResult(
			...
			out ToolResult failure)
		{
			...
			foreach (string objectId in normalizedIds)
			{
				...
				if (!TryResolveSaveEvaluationReflectionProbe(objectId, out probe) ||
					IdentityCompatibility.GetSceneToken(probe.gameObject.scene) != IdentityCompatibility.GetSceneToken(scene))
				{
					failure = CreateResult(
						"graphics.prepare_bake_plan",
						requestId,
				...
				if (!DependencyBakeSession.HasDirtyDependency(
					scene.path,
					E_GRAPHICS_BAKE_DEPENDENCY_KIND.REFLECTION_PROBE.ToString(),
					objectId))
				{
			...
			out List<Issue> issues)
		{
			...
			foreach (BakeSceneBaseline baseline
		...
		private static bool PreflightDependencyBakeDependencies(
			IEnumerable<PreparedBakeDependency> dependencies,
			out List<Issue> issues)
		{
			issues = new List<Issue>();
			foreach (PreparedBakeDependency dependency
		...
		{
			if (DependencyBakeSession.SceneBakeOverrideForTests != null)
			{
				return true;
			}
			...
			string outputAssetPath)
		{
			...
			builder.Append(IdentityCompatibility.GetSceneToken(probe.gameObject.scene)).Append('|');
			...
			ExecutableBakePlan plan)
		{
			...
			foreach (BakeSceneBaseline scene
				in plan.ContributingScenes
		...
		private static List<Dictionary<string, object>>
			BuildDependencyBakeScenePreviews(
				IEnumerable<BakeSceneBaseline> scenes)
		{
			return scenes
		...
		private static Issue CreateDependencyBakeIssue(
			string code,
			string message,
			object evidence)
		{
