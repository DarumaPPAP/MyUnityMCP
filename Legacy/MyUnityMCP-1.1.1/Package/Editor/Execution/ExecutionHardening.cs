...
namespace UnityGraphicsMcp
{
	...
	public sealed class ExecutionRecord
	{
		...
		public List<ProgressEvent> progressEvents { get; set; } =
			new List<ProgressEvent>();
	}
	...
	internal sealed class ExecutionScope
	{
		public string ExecutionId { get; set; }
	...
	[InitializeOnLoad]
	internal static class ExecutionHardening
	{
		...
		private static readonly Dictionary<string, ExecutionRecord> _active =
			new Dictionary<string, ExecutionRecord>(StringComparer.Ordinal);
		private static readonly List<ExecutionRecord> _history =
			new List<ExecutionRecord>();
		private static readonly Dictionary<string, ErrorCatalogEntry> _errorCatalog =
			BuildErrorCatalog();

		internal static Func<DateTime> UtcNowOverrideForTests { get; set; }
		internal static string StorageRootOverrideForTests { get; set; }

		static ExecutionHardening()
		{
			EnsureStorage();
			LoadHistory();
		...
		public static ExecutionScope Begin(
			string tool,
			string requestId,
			...
			string clientId = null)
		{
			...
			record.progressEvents.Add(new ProgressEvent
			{
		...
		public static ToolResult Complete(
			...
			ToolResult result)
		{
			...
			ExecutionRecord completedRecord = null;
			lock (_sync)
			{
			...
			out ExecutionRecord record)
		{
			...
			lock (_sync)
			{
				...
				ExecutionRecord history = _history
		...
		public static List<ExecutionRecord> GetHistory(
			string tool,
			int maxEntries)
		{
			...
			lock (_sync)
			{
				IEnumerable<ExecutionRecord> query = _history;
		...
		public static List<ErrorCatalogEntry> GetErrorCatalog()
		{
		...
		public static ToolResult GetExecutionStatus(
			string requestId,
			string executionId)
		{
			return Inspection.ExecuteHardeningReadOnly(
				...
				delegate
				{
					...
					{
						return Inspection.CreateHardeningResult(
							"graphics.get_execution_status",
							requestId,
		...
		public static ToolResult CancelExecution(
			string requestId,
			string executionId,
			string reason)
		{
		...
		public static ToolResult GetExecutionHistory(
			string requestId,
			string tool,
			int maxEntries)
		{
			return Inspection.ExecuteHardeningReadOnly(
				...
				delegate
				{
					List<ExecutionRecord> records = GetHistory(tool, maxEntries);
					return Inspection.CreateHardeningResult(
						"graphics.get_execution_history",
						requestId,
						E_MCP_TOOL_STATUS.SUCCESS,
		...
		public static ToolResult GetErrorCatalog(string requestId)
		{
			return Inspection.ExecuteHardeningReadOnly(
		...
		public static ToolResult GetSupportMatrix(string requestId)
		{
			return Inspection.ExecuteHardeningReadOnly(
		...
		private static void Tick()
		{
			...
			lock (_sync)
			{
				foreach (ExecutionRecord record in _active.Values)
				{
					DateTime started;
		...
		private static void InterruptMatching(
			Func<ExecutionRecord, bool> predicate,
			string code,
			string message)
		{
			List<string> ids;
		...
		private static void RecoverInterruptedExecutions(string code)
		{
			...
			try
			{
				List<ExecutionRecord> records =
					JsonConvert.DeserializeObject<List<ExecutionRecord>>(
						File.ReadAllText(path));
				if (records != null)
				{
					foreach (ExecutionRecord record in records)
					{
		...
		private static ExecutionMetadata BuildMetadata(
			ExecutionRecord record)
		{
		...
		private static StructuredError BuildStructuredError(
			ToolResult result,
			string code)
		{
		...
		private static string ResolveFailureCode(ToolResult result)
		{
			if (result == null || result.IsSuccessful)
			{
				return null;
			...
			Issue issue = result.issues == null
				? null
		...
		private static Dictionary<string, ErrorCatalogEntry> BuildErrorCatalog()
		{
		...
		private static ErrorCatalogEntry Catalog(
			string code,
			string category,
			bool retryable,
			string retryAction,
		...
		private static ErrorCatalogEntry CloneErrorCatalogEntry(
			ErrorCatalogEntry item)
		{
			return Catalog(
				item.code,
				item.category,
		...
		private static ExecutionRecord CreateDetachedRecord(
			string executionId,
			ToolResult result)
		{
		...
		private static ExecutionRecord CloneRecord(
			ExecutionRecord record)
		{
			return JsonConvert.DeserializeObject<ExecutionRecord>(
		...
		private static void AppendHistory(ExecutionRecord record)
		{
		...
		private static void AppendStructuredLog(ExecutionRecord record)
		{
			AppendJsonLine(
				StructuredLogPath(),
				new Dictionary<string, object>
		...
		private static void LoadHistory()
		{
			...
			try
			{
				foreach (string line in File.ReadLines(path))
				{
					...
					ExecutionRecord record =
						JsonConvert.DeserializeObject<ExecutionRecord>(line);
					if (record != null)
					{
						_history.Add(record);
					}
	...
	{
		internal static ToolResult ExecuteHardeningReadOnly(
			string toolName,
			string requestId,
		...
		internal static ToolResult CreateHardeningResult(
			string toolName,
			string requestId,
